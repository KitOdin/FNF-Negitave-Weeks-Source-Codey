...
class StoryMenuState extends MusicBeatState
{
	...
	var weekSongNames:Array<Dynamic> = [
		['Tutorial', ' '],
		['Bopeebo', 'Fresh', 'Dadbattle', ' '],
		['Spookeez', 'South', ' '],
	...
	override function create()
	{
		...
		scoreText.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreText.screenCenter(X);

		txtWeekTitle = new FlxText(FlxG.width * 0.7, 46, 0, "", 32);
		txtWeekTitle.setFormat("VCR OSD Mono", 32, FlxColor.RED, CENTER);
		...
		yellowBG = new FlxSprite(0, 0).loadGraphic('assets/images/menuBGGreen.png');
		...
		var blackBarThingieB:FlxSprite = new FlxSprite(0, 456).makeGraphic(FlxG.width, (FlxG.height - 456), FlxColor.BLACK);
		
		var Overlay:FlxSprite = new FlxSprite(0, 0).loadGraphic('assets/images/ui/menuOverlay.png');
		...
		add(blackBarThingieB);
		add(Overlay);
		add(grpWeekText);
		add(difficultySelectors);

		txtTracklist = new FlxText(FlxG.width * 0.05, yellowBG.x + 500, 0, "Tracks", 32);
	...
	function selectWeek()
	{
		if (weekUnlocked[curWeek])
		{
			if (stopspamming == false)
			{
				...
				if (curWeek == 8)
				{
					grpWeekCharacters.members[0].animation.play('blackConfirm');
				}
				stopspamming = true;
			}
	...
	function updateText()
	{
		...
		switch (grpWeekCharacters.members[0].animation.curAnim.name)
		{
			...
			case 'head':
				grpWeekCharacters.members[0].offset.set(75, -50);
				grpWeekCharacters.members[0].setGraphicSize(Std.int(grpWeekCharacters.members[0].width * 1.5));
			
			case 'black':
				grpWeekCharacters.members[0].offset.set(10, -50);
		...
		var stringThing:Array<String> = weekSongNames[curWeek];

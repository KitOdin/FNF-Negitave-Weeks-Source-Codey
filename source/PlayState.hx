...
class PlayState extends MusicBeatState
{
	...
	public static var OldMode:Bool;
	...
	public static var AgonyMod:Bool = false;
	public static var FovMaxMod:Bool = false;
	public static var SlownessMod:Bool = false;
	public static var wutMode:Bool = false;
	...
	var endDia:Array<String> = ['blah blah blah', 'coolswag'];
	...
	override public function create()
	{
		...
		SONG.speed += Prefrences.speedOffset;
		if (SlownessMod)
		{
			SONG.speed = 0.1;
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'ridge':
				dialogue = CoolUtil.coolTextFile('assets/data/ridge/ridgeDialouge.txt');
			case 'vidya':
				dialogue = CoolUtil.coolTextFile('assets/data/vidya/vidyaDialouge.txt');
			case 'lockers':
				dialogue = CoolUtil.coolTextFile('assets/data/lockers/lockersDialouge.txt');
			case 'void':
				dialogue = CoolUtil.coolTextFile('assets/data/void/voidDialouge.txt');
			case 'colors':
				dialogue = CoolUtil.coolTextFile('assets/data/colors/colorsDialouge.txt');
			case 'test':
				dialogue = CoolUtil.coolTextFile('assets/data/test/testDialouge.txt');
				endDia = CoolUtil.coolTextFile('assets/data/test/goof.txt');
		}
		...
		else if (SONG.song.toLowerCase() == 'ridge' || SONG.song.toLowerCase() == 'vidya' || SONG.song.toLowerCase() == 'lockers')
		{
			curStage = 'emptyroom';

			defaultCamZoom = 1.2;

			var bg:FlxSprite = new FlxSprite(-128, -72).loadGraphic('assets/images/negitave/RandomRoom.png');
			bg.antialiasing = true;
			bg.scrollFactor.set(0.7, 0.7);
			bg.active = false;
			GrpSprites.add(bg);
			
			bottomBoppers = new FlxSprite(737, 435);
			bottomBoppers.frames = FlxAtlasFrames.fromSparrow('assets/images/negitave/SpeakerBod.png', 'assets/images/negitave/SpeakerBod.xml');
			bottomBoppers.animation.addByPrefix('bop', 'SpeakerBoi', 24, false);
			bottomBoppers.antialiasing = true;
			bottomBoppers.scrollFactor.set(0.9, 0.9);
			GrpSprites.add(bottomBoppers);
		}
		
		else if (SONG.song.toLowerCase() == 'void' || SONG.song.toLowerCase() == 'colors' || SONG.song.toLowerCase() == 'test')
		{
			curStage = 'outside';
			...
			var fg:FlxSprite = new FlxSprite(-455, -200).loadGraphic('assets/images/negitave/outside/outsideFG.png');
			fg.antialiasing = true;
			fg.scrollFactor.set(0.9, 0.9);
			fg.active = false;
			GrpSprites.add(fg);
			
			bottomBoppers = new FlxSprite(649, 368);
			bottomBoppers.frames = FlxAtlasFrames.fromSparrow('assets/images/negitave/outside/bopperz.png', 'assets/images/negitave/outside/bopperz.xml');
			bottomBoppers.animation.addByPrefix('bop', 'bopbop', 24, false);
			bottomBoppers.antialiasing = true;
			bottomBoppers.scrollFactor.set(0.8, 0.8);
			GrpSprites.add(bottomBoppers);
		}
		else if (SONG.song.toLowerCase() == 'funkin-what')
		{
			defaultCamZoom = 1;
			curStage = 'nonsense';
			var bg:FlxSprite = new FlxSprite(-320, -180).loadGraphic('assets/images/negitave/swirlySwirlySWIRLY.png');
			bg.antialiasing = true;
			...
			var meme:FlxSprite = new FlxSprite(133, 561).loadGraphic('assets/memes/Amogus.png');
			meme.scrollFactor.set(0.6, 0.6);
			
			var fnfMeme:FlxSprite = new FlxSprite(913, 123).loadGraphic('assets/memes/FNNF.png');
			fnfMeme.scrollFactor.set(0.8, 0.8);
			
			GrpSprites.add(meme);
			GrpSprites.add(fnfMeme);
			someCharacter = new FlxSprite(720, 375).loadGraphic('assets/images/negitave/pain.png');
			someCharacter.antialiasing = true;
			someCharacter.scrollFactor.set(0.9, 0.9);
			someCharacter.visible = false;
			GrpSprites.add(someCharacter);
		}
		else
		{
			defaultCamZoom = 0.9;
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'outside':
				dad.setPosition(106, 409);
				if (dad.curCharacter == "head")
				{
					dad.y -= 50;
				}
				gf.setPosition(377, 127);
				boyfriend.setPosition(881, 409);
			case 'emptyroom':
				dad.setPosition(210, 375);
				gf.setPosition(397, 157);
				boyfriend.setPosition(737, 375);
				if (dad.curCharacter == "lucky")
				{
					dad.y -= 275;
					dad.x -= 400;
				}
			case 'nonsense':
				dad.setPosition(720, 375);
				gf.setPosition(460, 375);
				boyfriend.setPosition(100, 375);
				
		}

		GrpSprites.add(gf);
		GrpSprites.add(dad);
		GrpSprites.add(boyfriend);
		add(GrpSprites);
		var doof:DialogueBox = new DialogueBox(false, dialogue);
		trace('Duf');
		duuf = new DialogueBox(false, endDia);
		trace('duf daf n duffy');
		// doof.x += 70;
		// doof.y = FlxG.height * 0.5;
		doof.scrollFactor.set();
		doof.finishThing = startCountdown;
		duuf.scrollFactor.set();
		duuf.finishThing = beepBopCutscene;
		...
		songInfoText = new FlxText(0, (FlxG.height - 60), 0, "loading song info... \n please wait a while...", 20);
		songInfoText.alignment = LEFT;
		songInfoText.font = "assets/fonts/vcr.ttf";
	...
	function endDialouge(?dialogueBox:DialogueBox):Void
	{
		if (dialogueBox != null)
		{
			add(dialogueBox);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var BGrand:FlxSprite = new FlxSprite( -500, -500).loadGraphic('assets/images/negitave/RandomRoom.png');
		BGrand.setGraphicSize(Std.int(BGrand.width * 2));
		BGrand.updateHitbox();
		BGrand.screenCenter();
		
		var lockeyScene:FlxSprite = new FlxSprite( -150, 41);
		lockeyScene.frames = FlxAtlasFrames.fromSparrow('assets/images/negitave/cutscenes/LockyCutscene.png', 'assets/images/negitave/cutscenes/LockyCutscene.xml');
		lockeyScene.animation.addByPrefix('chacha', 'LockyCutscene', 24, false);
		...
		if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'thorns'|| SONG.song.toLowerCase() == 'void')
		{
			remove(black);
			...
			else if (SONG.song.toLowerCase() == 'void')
			{
				add(BGrand);
				add(lockeyScene);
			}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (SONG.song.toLowerCase() == 'void')
					{
						lockeyScene.animation.play('chacha');
						new FlxTimer().start(1.45, function(swagTimer:FlxTimer)
						{
							FlxG.sound.play('assets/sounds/bunchObeepBoops' + TitleState.soundExt, 1);							
						});
						new FlxTimer().start(2.45, function(swagTimer:FlxTimer)
						{
							FlxG.sound.play('assets/sounds/bbMonolouge' + TitleState.soundExt, 1);						
						});
						new FlxTimer().start(7.65, function(swagTimer:FlxTimer)
						{
							remove(BGrand);
							remove(lockeyScene);
							add(dialogueBox);
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			introAssets.set('nonsense', ['ui/pause.png', "restart.png", "ready.png"]);
...

			{
				case 0:
					...
					{
						FlxG.sound.play('assets/sounds/GF_3' + TitleState.soundExt, 1);
					}
					else
					{
				...
				case 1:
					...
					if (curStage == 'nonsense')
					{
						FlxG.sound.play('assets/sounds/GF_2' + TitleState.soundExt, 1);
					}
					else
					{
				...
				case 2:
					...
					if (curStage == 'nonsense')
					{
						FlxG.sound.play('assets/sounds/GF_1' + TitleState.soundExt, 1);
					}
					else
					{
				...
				case 3:
					...
					if (curStage == 'nonsense')
					{
						FlxG.sound.play('assets/sounds/GF_4' + TitleState.soundExt, 1);
					}
					else
					{
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'nonsense':
					...
					babyArrow.frames = FlxAtlasFrames.fromSparrow('assets/images/ui/weirdArrows.png', 'assets/images/ui/weirdArrows.xml');
	...
	override public function update(elapsed:Float)
	{
		...
		if (dad.curCharacter == "lol" && SONG.notes[Math.floor(curStep / 16)] != null)
		{
			dad.AltMode = SONG.notes[Math.floor(curStep / 16)].altAnim;
		}
		...
		{
			if (iconP1.animation.curAnim.name == 'bf-grafix')
				iconP1.animation.play(SONG.player1);
			else
				iconP1.animation.play('bf-grafix');
		}
		
		if (FlxG.keys.justPressed.SIX)
		{
			if (iconP1.animation.curAnim.name == 'head')
				iconP1.animation.play(SONG.player1);
			else
				iconP1.animation.play('head');
		}
		
		if (FlxG.keys.justPressed.FOUR)
		{
			if (iconP1.animation.curAnim.name == customIconName)
				iconP1.animation.play(SONG.player1);
			else
				iconP1.animation.play(customIconName);
		}
		...
		scoreTxt.text = "Score:" + songScore + " Highest Combo:" + highCombo + " Misses:" + misses + " Slip-Ups:" + slipups;
		songInfoText.text = "Song name : " + SONG.song + "\n" + "BPM : " + Conductor.bpm + "\n" + " ";
		...
		if (controls.CHEAT && curSong == 'Funkin-What')
		{
			health += 1;
			//NO TRACE MESSAGE HERE FOR SHIT SAKE
		}
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					//trace line removed for anti-spam measurement
				...
				//Kade engine is broken
				//daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) *  FlxMath.roundDecimal(PlayState.SONG.speed, 2));
				if (FlxG.save.data.downscroll)
				{
	...
	function endSong():Void
	{
		...
		{
			if (curSong == "Funkin-What")
			{
				FlxG.switchState(new SecretEndState());
			}
			else
			{
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		if (curStage.startsWith('nonsense'))
		{
			pixelShitPart1 = 'wut/';
			pixelShitPart2 = '';
		}
		...
		{
			rating.acceleration.x = FlxG.random.int(-300, 300);
			rating.acceleration.y = 1050;
			rating.velocity.y -= FlxG.random.int(1025, 1075);
			rating.velocity.x -= FlxG.random.int(-300, 300);
		}
		...
		for (i in seperatedScore)
		{
			...
			{
				numScore.velocity.x = FlxG.random.float( -50, 50);
				numScore.velocity.y -= FlxG.random.int(240, 260);
			}
	...
	function noteMiss(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (curSong == "Funkin-What")
			{
				...
				{
					FlxG.sound.play('assets/sounds/badnoise' + FlxG.random.int(1, 3) + TitleState.soundExt, 1);
				}
			}
			else
			{
	...
	override function beatHit()
	{
		...
		if (curSong == 'Funkin-What')
		{
			var scre:FlxSound = new FlxSound().loadEmbedded('assets/sounds/bassBoost' + TitleState.soundExt);
			...
			switch (totalBeats)
			{
				...
				case 48:
					...
					FlxG.camera.zoom = 1;
					dad.visible = false;
					someCharacter.visible = true;
				case 49:
					scre.stop();
					FlxG.camera.zoom = 0.9;
					boyfriend.setPosition(720, 475);
					dad.visible = true;
					someCharacter.visible = false;
				case 69:
					if (FlxG.random.bool(1))
					{
						FlxG.sound.play('assets/sounds/funni' + TitleState.soundExt);
					}
		...
		if (FlxG.random.bool(10) && totalBeats % 8 == 7 && dad.curCharacter == 'head')
		{
		...
		if (totalBeats % 8 == 7)
		{
			if (curSong == 'Tutorial' || curSong == 'Vidya')
			{
				gf.playAnim('cheer', true);
			}
			
		}
		if (FlxG.random.bool(10) && dad.curCharacter == 'lucky')
		{
			dad.playAnim("tumble", true);
		}
		switch (curStage)
		{
			case 'school':
